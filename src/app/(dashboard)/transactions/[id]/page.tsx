...
const mockTransactionDetail: Transaction = {
  ...
  description: 'Transfer ke rekening bisnis untuk pembayaran invoice #INV-2025-001. Pembayaran untuk kontrak jasa konsultasi teknologi periode Januari 2025.',
...
const transactionTimeline = [
  {
    ...
    details: 'Transaction automatically blocked by fraud detection system due to high risk score (95%)',
  ...
  },
  {
    ...
    details: 'ML model analysis completed with 95% confidence score. Multiple risk factors detected.',
  ...
  },
  {
    ...
    details: 'Unknown device detected. Device fingerprint does not match known trusted devices.',
...
export default function TransactionDetailPage({ params }: TransactionDetailPageProps) {
  ...
  const [transaction, setTransaction] = useState<Transaction>(mockTransactionDetail);
  ...
  const getTypeIcon = (type: TransactionType) => {
    switch (type) {
      case TransactionType.TRANSFER: return <ArrowRightIcon className="h-5 w-5" />;
      case TransactionType.PAYMENT: return <CreditCardIcon className="h-5 w-5" />;
      case TransactionType.WITHDRAWAL: return <ArrowUpIcon className="h-5 w-5" />;
      case TransactionType.DEPOSIT: return <ArrowDownIcon className="h-5 w-5" />;
  ...
  const handleTransactionAction = (action: string) => {
    ...
    console.log(`Performing action: ${action} on transaction ${transaction.transactionId}`);
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              {formatDateTime(transaction.transactionTime)} • {formatRelativeTime(transaction.transactionTime)}
            </p>
          </div>
        </div>
        
        <div className="flex items-center space-x-2">
          <Badge variant={getStatusColor(transaction.status) as any} className="flex items-center gap-1">
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {transaction.status === TransactionStatus.BLOCKED && (
                      <>
                        <DropdownMenuItem onClick={() => handleTransactionAction('approve')}>
                          <CheckCircleIcon className="h-4 w-4 mr-2" />
                          Approve Transaction
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleTransactionAction('reject')}>
                    ...
                    {transaction.status === TransactionStatus.PENDING && (
                      <DropdownMenuItem onClick={() => handleTransactionAction('cancel')}>
                ...
                {transaction.fees && transaction.fees.length > 0 && (
                  <div className="mt-3 space-y-1">
                    ...
                    <p className="text-sm font-medium">
                      Total: {formatCurrency(transaction.amount + transaction.fees.reduce((sum, fee) => sum + fee.amount, 0))}
                    </p>
                  </div>
                )}
              </div>
          ...
          <Tabs defaultValue="risk-analysis" className="w-full">
            <TabsList className="grid w-full grid-cols-4">
              ...
              <TabsTrigger value="location-device">Location & Device</TabsTrigger>
                ...
                <CardContent>
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <Progress value={transaction.riskScore! * 100} className="h-3" />
                    ...
                    {transaction.riskAssessment && (
                      <div className="space-y-4">
                        ...
                        {Object.entries(transaction.riskAssessment.factors).map(([factor, value]) => (
                          <div key={factor} className="space-y-2">
                            <div className="flex items-center justify-between">
                              <span className="text-sm font-medium capitalize">
                                {factor.replace(/([A-Z])/g, ' $1').trim()}
                              </span>
                              <span className={`text-sm font-semibold ${getRiskFactorColor(value)}`}>
                                {(value * 100).toFixed(1)}%
                              </span>
                            </div>
                    ...
                    {transaction.riskAssessment?.recommendations && (
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                          {transaction.riskAssessment.recommendations.map((recommendation, index) => (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3 pl-6">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{transaction.location?.region}</p>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">City:</span>
                      <p className="font-medium">{transaction.location?.city}</p>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">IP Address:</span>
                      <p className="font-mono text-sm">{transaction.location?.ipAddress}</p>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">Timezone:</span>
                      <p className="text-sm">{transaction.location?.timezone}</p>
                    </div>
                  ...
                  <div className="space-y-3 pl-6">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <p className="font-medium">{transaction.deviceInfo?.deviceType}</p>
                      </div>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">Operating System:</span>
                      <p className="font-medium">{transaction.deviceInfo?.os}</p>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">Browser/App:</span>
                      <p className="font-medium">{transaction.deviceInfo?.browser}</p>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">Device ID:</span>
                      <p className="font-mono text-sm">{transaction.deviceInfo?.deviceId}</p>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">Device Fingerprint:</span>
                      <p className="font-mono text-sm">{transaction.deviceInfo?.fingerprint}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-muted-foreground">Trusted:</span>
                        <Badge variant={transaction.deviceInfo?.isTrusted ? 'default' : 'destructive'}>
                          {transaction.deviceInfo?.isTrusted ? 'Yes' : 'No'}
                        </Badge>
                      </div>
                      {transaction.deviceInfo?.isJailbroken !== undefined && (
                        <div className="flex items-center gap-2">
                          <span className="text-sm text-muted-foreground">Jailbroken:</span>
                          <Badge variant={transaction.deviceInfo.isJailbroken ? 'destructive' : 'default'}>
                            {transaction.deviceInfo.isJailbroken ? 'Yes' : 'No'}
                          </Badge>
                        </div>
                      )}
                    </div>
                ...
                {transactionTimeline.map((event, index) => (
                  <div key={event.id} className="flex items-start gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`w-3 h-3 rounded-full ${getTimelineStatusColor(event.status)}`} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <span className="text-sm text-muted-foreground">Risk Assessment Date:</span>
                      <p className="text-sm">
                        {transaction.riskAssessment?.computedAt ? 
                          formatDateTime(transaction.riskAssessment.computedAt) : 'N/A'
                        }
                      </p>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">User Agent:</span>
                      <p className="font-mono text-xs break-all">{transaction.deviceInfo?.userAgent}</p>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <Separator />
          <Button variant="outline" className="w-full" onClick={handleViewAlert}>
            <ExclamationTriangleIcon className="h-4 w-4 mr-2" />
            View Related Alert
          </Button>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold">
              {Math.floor((new Date(transaction.updatedAt).getTime() - new Date(transaction.createdAt).getTime()) / 1000)}s
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-sm text-muted-foreground">Channel</span>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">{senderAccount.accountNumber}</p>
              <p className="text-xs text-muted-foreground">{senderAccount.bankName}</p>
              <Badge variant="outline" className="text-xs">
          ...
          <div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">{receiverAccount.accountNumber}</p>
              <p className="text-xs text-muted-foreground">{receiverAccount.bankName}</p>
              <Badge variant="outline" className="text-xs">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Recent transactions from sender account</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {relatedTransactions.map((txn) => (
              <div key={txn.id} className="flex items-center justify-between p-2 border border-border rounded">
                <div>
                  <p className="text-sm font-medium">{txn.id}</p>
                  <p className="text-xs text-muted-foreground">{txn.description}</p>
                  <p className="text-xs text-muted-foreground">{formatRelativeTime(txn.time)}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold">{formatCurrency(txn.amount)}</p>
                  <Badge variant={getStatusColor(txn.status as TransactionStatus) as any} className="text-xs">
                    {txn.status}
                  </Badge>
                </div>
              </div>
    ...
    <DialogContent>
      ...
      <div className="space-y-4">
        <div className="p-4 border border-border rounded-lg bg-accent/5">
          ...
          <p className="text-sm text-muted-foreground">Amount: {formatCurrency(transaction.amount)}</p>
          <p className="text-sm text-muted-foreground">From: {senderAccount.holderName}</p>
          <p className="text-sm text-muted-foreground">To: {receiverAccount.holderName}</p>
        </div>
